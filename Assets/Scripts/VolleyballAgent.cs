...
public class VolleyballAgent : Agent
{
    ...
    public GameObject agentTeammate;
    Rigidbody agentTeammateRB;
    public Collider center;
    private Bounds bounds;

    [SerializeField] private float jumpPunishment;
    private bool hasHitTheBall =false;
    ...
    public override void Initialize()
    {
        ...
        agentTeammateRB = agentTeammate.GetComponent<Rigidbody>();
        ...
        Vector3 targetPos, Rigidbody rb, float targetVel, float maxVel)
    {
        ...
        if (float.IsNaN(velocityTarget.x) == false)
        {
            rb.velocity = Vector3.MoveTowards(
                rb.velocity, velocityTarget, maxVel*2);
    ...
    void OnCollisionEnter(Collision c)
    {
        ...
        {
            envController.UpdateLastHitter(teamId,agentId);
            if(didJump){
                hasHitTheBall = true;
            }
    ...
    public void MoveAgent(ActionSegment<int> act)
    {
        ...
        // Jumping Punsihment
        if(didJump){
            AddReward(jumpPunishment);
            //Debug.Log(jumpPunishment);
            didJump = false;
            //hasHitTheBall = false;
        }
        /* 
        // (OLD APPROACH) If agent jumped and hit the ball, return the penalty
        if(grounded && didJump && hasHitTheBall){
            AddReward(0.2f);
            //Debug.Log(jumpPunishment);
            didJump = false;
            hasHitTheBall = false;
        }
        else if(grounded && didJump && !hasHitTheBall){
            didJump = false;
        }
        */
    ...
    //OLD APPROACH -> PUNISH AGENT FOR CLOSE CONTACT WITH TEAM MEMEBER
    //     //Vector from Agent to Teammate (3 float)
    //     Vector3 toTeamMate = new Vector3((agentTeammateRB.transform.position.x - this.transform.position.x)*agentRot, 
    //     (agentTeammateRB.transform.position.y - this.transform.position.y),
    //     (agentTeammateRB.transform.position.z - this.transform.position.z)*agentRot);
        
    //     sensor.AddObservation(toTeamMate.normalized);

    //    //Distance from Teammate (1 float)
    //     sensor.AddObservation(toTeamMate.magnitude);
        
    //     //Velocity of Teammate(3)
    //     sensor.AddObservation(agentTeammateRB.velocity);

    //      Jump observation
    //      sensor.AddObservation(jumpingTime>0);

        Vector3 closestPoint = bounds.ClosestPoint(transform.position);
        float distanceFromCenter =  Mathf.Max((Vector3.Distance(transform.position,closestPoint) - 1)/14f,0);
        //Dsitance from center (1 float)
        sensor.AddObservation(distanceFromCenter);
        //Position observation (1 float)
